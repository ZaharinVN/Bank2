...
class Bank {
    ...
    public BankAccount createNewAccount(String type, String currency) { // создать метод createNewAccount, который принимает на вход строку с типом аккаунта и строку с создаваемой валютой
        switch (type) {
            case "debit_account": { // если тип "debit_account"
                ...
                case "credit_account" : { // если тип "credit_account"
                    int creditLimit = getCreditLimit(currency); // посчитать кредитный лимит в зависимости от валюты
                ...
                default: {// иначе
                    ...
                    break;// создать пустой объект BankAccount()
                }
            }
        }
    }
    public BankAccount closeAccount(String BankAccount) { // создать метод closeAccount, который принимает на вход переменную типа BankAccount
        switch (BankAccount){
            case "DebitAccount": {// если переданный аккаунт дебетовый
                if (amount = 0) {// если на счету нет денег вывести сообщение "Ваш дебетовый счет закрыт"
                    ...
                    System.out.println("Ваш дебетовый счет закрыт. Вы можете получить остаток по вашему счету в размере " + amount + " " + currency + " в отделении банка"); // иначе вывести сообщение "Ваш дебетовый счет закрыт. Вы можете получить остаток по вашему счету в размере {amount} {currency} в отделении банка"
                }
            }
            case "CreditAccount": { // если переданный аккаунт кредитный
                if (amount = 0) {
                    System.out.println("Ваш кредитный счет закрыт"); // если на счету нет денег вывести сообщение "Ваш кредитный счет закрыт"
                }
                else if (amount > 0){
                    System.out.println("Ваш кредитный счет закрыт. Вы можете получить остаток по вашему счету в размере " + amount + " " + currency + " в отделении банка"); // если на счету положительный баланс вывести сообщение "Ваш кредитный счет закрыт. Вы можете получить остаток по вашему счету в размере {amount} {currency} в отделении банка"
                }
                else if (creditAccount.amount < 0){
                    System.out.println("Вы не можете закрыть кредитный счет потому как на нем еще есть задолженность. Ваша задолженность по счету составляет " + amount + " " + currency);  // если на счету отрицательный баланс вывести сообщение "Вы не можете закрыть кредитный счет потому как на нем еще есть задолженность. Ваша задолженность по счету составляет {amount} {currency}"
                }
            }
            default:{
